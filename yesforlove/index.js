...
(async () => {
    ...
    const mainUrl = 'https://www.yesforlov.com/collections/tous-les-produits';
    ...
    const filename = 'yesforlove-p1-product.json';
    
    const cookieAcceptBtn = await page.$("#axeptio_btn_acceptAll");

    await console.log('The element cookieAcceptBtn was resolved to: ' + cookieAcceptBtn);
    await cookieAcceptBtn.click();

    // fermer la page pour selectionner la langue
    await page.waitForSelector('body > div:nth-child(42) > div > div > div > div > div > div > button > svg > path', {timeout: 100000});

    // Attendre que le selector des liquides charge
    await page.waitForSelector('#shopify-section-collection-template > section > div.Container > div > div > div > div.ProductListWrapper > div > div> div > div > a ', {timeout: 100000});

    // Stock toutes les infos des buttons vers les collections
    const products = await page.$$('#shopify-section-collection-template > section > div.Container > div > div > div > div.ProductListWrapper > div > div> div > div > a ');
